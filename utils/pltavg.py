...
@app.command()
def main(fname: Ann[str, typer.Argument(help="the filename of the csv file whose columns will be averaged and plotted. If omitted will read from stdin.")]=None,
         ...
         index_col:Ann[str, typer.Option("--index", "-i", help="If set, provides the name of the column that will be used as index to the data. The table must have valid headers and the has-header must be used. If not set a default 0..N integer index will be used.")] = None):
    """
    ...
    fname = fname if fname else sys.stdin    
